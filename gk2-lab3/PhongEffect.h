	...
	namespace gk2
	{
		...
		public:
			...
			PhongEffect(dx_ptr<ID3D11VertexShader>&& vs, dx_ptr<ID3D11PixelShader>&& ps, const ConstantBuffer<DirectX::XMFLOAT4X4>& cbWorld,
				...
				const ConstantBuffer<DirectX::XMFLOAT4, 2>& cbLightPos, const ConstantBuffer<DirectX::XMFLOAT4>& cbSurfaceColor)
			...
			void SetLightPosBuffer(const ConstantBuffer<DirectX::XMFLOAT4, 2>& buffer) {
